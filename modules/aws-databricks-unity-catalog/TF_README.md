...
|------|-------------|------|---------|:--------:|
| <a name="input_existing_metastore_id"></a> [existing\_metastore\_id](#input\_existing\_metastore\_id) | ID of the existing metastore to reuse (required when reuse\_metastore = true) | `string` | `null` | no |
...
| <a name="input_reuse_metastore"></a> [reuse\_metastore](#input\_reuse\_metastore) | If true, looks up the existing metastore by ID instead of creating a new one | `bool` | `false` | no |
| <a name="input_storage_configuration_role_arn"></a> [storage\_configuration\_role\_arn](#input\_storage\_configuration\_role\_arn) | IAM role ARN for storage configuration | `string` | n/a | yes |
